...
class target(motion_model):
    def __init__(self, initial_location, mean_x_vel, mean_y_vel, x_var, y_var,motion_type, sample_time, rate):
        motion_model.__init__(self,sample_time)
        ...
        self.rate = rate
        self.motion_params = {}
        if motion_type==self.constant_velocity_type:
            A, B = self.constant_velocity(self.rate)
            self.motion_params["A"] = A
            self.motion_params["B"] = B
    ...
    def update_location(self):
        ...
        if self.motion_type==self.constant_velocity_type:
            A = self.motion_params["A"]
            B = self.motion_params["B"]
