...
class JPDAF_agent:
    ...
    def get_gate_map(self,measurements):
        num_targets = len(self.tracks)  # number of tracks
        ...
        for t in range(0, num_targets):
            ...
            for m in range(0, num_measurements + 1):
                if m == 0:
                    ...
                    meas = np.array(measurements[m - 1])
                    if not self.use_velocity: meas = meas[0:2]
                    ...
                    distance = innov.transpose().dot(np.linalg
                                                          .inv(self.tracks[t].S_k_R)).dot(innov)
                    ...
                    if distance < self.threshold:
                        if self.use_velocity:
                            ...
                            reshape_size = 2
                        assignment_score = multivariate_normal.pdf(innov.reshape(reshape_size), mean=np.zeros(reshape_size),
                                                                   cov=self.tracks[t].S_k_R, allow_singular=True)/self.landa

                        #print(m,t,assignment_score)
    ...
    def update_target_states(self,sensor_state,measurements,target_measurement_prob, tracks_to_update):
        ...
        for idx,track in enumerate(tracks_to_update):
            ...
            for meas_index,m in enumerate(measurements):
                if not self.use_velocity: m = m[0:2]
                m = np.array(m)
            ...
            track.innovation_var.append(track.S_k_R)
            #update target-state
            track.x_k_k = track.x_k_km1 + kalman_gain.dot(weighted_innov)
            #update covariance

            track.p_k_k =  no_meas_assignment_prob*track.p_k_km1+(1-no_meas_assignment_prob)*(track.p_k_km1 - (kalman_gain.dot(track.S_k_R)).dot(kalman_gain.transpose())) + \
