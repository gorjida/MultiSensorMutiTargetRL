...
class Distance_Cluster(static_params):
    def __init__(self,max_distance,points):
        ...
        self.max_distance = max_distance
        self.covariance = np.diag([100,15*np.pi/180,20])
...
from point_cloud_tracker import *

if __name__=="__main__":
    base_path = "/home/ali/SharedFolder/view_meas_June5th/"
    points = parse_ti_data(base_path,"sngl_stop.txt")
    #points = parse_ti_data(base_path, "s.txt")
    estimated_tracks, scan_based_tracks = main(points)
    ...
    for p in points:
        ...
        for f in filtered_clusters:
            c = f.centroid
            x = c.range*np.sin(c.azimuth)
            y = c.range*np.cos(c.azimuth)
            in_cluster_x.append(x)
            in_cluster_y.append(y)
        X.append(in_cluster_x)
        Y.append(in_cluster_y)
    ...
    for index,x in enumerate(X):
        if len(x)==0: continue
        for in_index,p in enumerate(x):
            ax1.scatter(p,Y[index][in_index])
            ax1.annotate(str(index),(p,Y[index][in_index]))

        for d_point in scan_based_tracks[index]:
            point =  d_point[0]
            id = d_point[1]
            ax2.scatter(point[0],point[1],marker=r"$\clubsuit$")
            ax2.annotate(str(id),(point[0],point[1]))
        plt.pause(.2)
        #if index>200: break
