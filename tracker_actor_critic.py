...
if __name__=="__main__":
    ...
    theta = np.random.normal(0, 1, [1, num_states]) #critic network weights (to estimate value function)
    ...
    while episode_counter<N_max:
        ...
        while episode_condition:
            ...
            current_action = s.sensor_actions[-1] #
            ...
            if not not prev_state:
                #actor-critic algorithm
                # list all the variables
                # s_{t+1} = current_state
                # s_t = prev_state
                # a_t = current_action
                # r_t = reward[-1]
                rate = gen_learning_rate(episode_counter, learning_rate, 1E-8, 10000)
                current_value = theta.dot(prev_state)
                delta = reward[-1] + gamma*theta.dot(current_state)-current_value
                #update theta
                theta = theta + rate*delta*prev_state.transpose()#update weights
                gradiant = (current_action.reshape(2,1)-weight.dot(prev_state)).dot(prev_state.tranpose())/sigma**2
                adjustment_term = gradiant*delta
                weight+= rate*adjustment_term
            prev_state = list(current_state)  # s_t
