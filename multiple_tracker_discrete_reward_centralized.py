...
x_max_pooling = 2
y_max_pooling = 2
output_size = int((num_states/(1.0*x_max_pooling))*(MAX_TARGETS/(1.0*y_max_pooling))) #size of flattened state

base_path = "/Users/u6042446/Downloads/DeepSensorManagement-original/"
...
def run(args):
    ...
    params ={0:{},1:{},2:{}}
    if method==0:
        params[0]["weight2"] = np.random.normal(0, .3, [2, output_size]) #Output-layer (maps flattened states to the actions)
        params[0]["weight"] = []
    ...
    while episode_counter<N_max:
        ...
        while episode_condition:
            ...
            input_state = np.zeros([num_states,MAX_TARGETS]) #create a fixed-size matrix for input states
            ...
            grad_with_out_state_w1,grad_with_out_state_w2,out_state = \
                s.convolution_featurizer(params, input_state, x_max_pooling, y_max_pooling,sigma)

            #Output created by the CNN
            episode_state_out_layer.append(out_state)
            #Gradiant of output state with respect to the weight matrices
            episode_grad_with_state_w1.append(grad_with_out_state_w1)
            episode_grad_with_state_w2.append(grad_with_out_state_w2)
        ...
        if episode_condition and training:
            ...
            for e in range(0,len(episode_actions)):
                ...
                #calculate gradient
                if method==0:
                    predicted_action = params[0]['weight2'].dot(out_state)
                    ...
                    #Calculate gradiant for the internal layer (channels weight)
                    gradiant_internal = []
                    for index,f in enumerate(params[0]["weight"]):
                        gw1 = episode_grad_with_state_w1[e][index]
                        gw2 = episode_grad_with_state_w2[e][index]

                        gradiant_internal.append(((episode_actions[e][0]-predicted_action[0])
                         *gw1+(episode_actions[e][1]-predicted_action[1])*gw2)/(sigma**2))
                ...
                if method==0:
                    ...
                    for index,f in enumerate(params[0]["weight"]):
                        params[0]["weight"][index]+= internal_rate*gradiant_internal[index]*normalized_discounted_return[e]
            ...
            episode_counter+=1
            flatted_weights = list(params[0]['weight2'][0,:]) + list(params[0]['weight2'][1,:])
            temp = []
            [temp.append(str(x)) for x in flatted_weights]
            weight_file.write("\t".join(temp)+"\n")
        ...
        prev_params[0]["weight2"] = .9*prev_params[0]["weight2"]+.1*params[0]["weight2"]
        for index,f in enumerate(prev_params[0]["weight"]):
            prev_params[0]["weight"][index] = .9*prev_params[0]["weight"][index]+.1*params[0]["weight"][index]
...
if __name__=="__main__":
    ...
    job_args = [(method,RBF_components,MLP_neurons,i,experiment_folder_name,vel_var,num_targets) for i in range(0,2)]
    #run(job_args[0])
    p.map(run,job_args)
