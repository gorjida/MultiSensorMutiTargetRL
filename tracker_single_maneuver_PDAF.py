...
class EKF_tracker:
    ...
    def predicted_state(self,sensor_state,measurement):
        ...
        temp = np.diag([1E9,1E9,1E9,1E9])
        self.S_k_max_uncertainty = (measurement_vector.dot(temp)).dot(measurement_vector.transpose()) + self.bearing_var
        self.innovation_list.append(measurement - predicted_measurement)
        self.innovation_var.append(self.S_k)
        self.innovation_var_max.append(self.S_k_max_uncertainty)
...
if __name__=="__main__":
    ...
    mean_false_alarm_num = 5
    pd = .9 #probability of target detection
    pg = 1 #probability of gate
   ...
   # weight = np.array([[  8.03624352,  -7.17570875,   1.58966854,  -8.86863398,
    #     -0.05360271,  10.43345794],
     #  [  7.46017625,   4.55351183,   7.34028265,   2.5319058 ,
      #   -7.31203658,  -1.04066541]])
    ...
    #result_folder = "/Users/u6042446/Downloads/DeepSensorManagement-original"
    ...
    while episode_counter<N_max:
        ...
        t = target(init_target_state[0:2], init_target_state[2], init_target_state[3], .002, .002, "CONS_V")#constant-velocity model for target motion
        ...
        while episode_condition:
            ...
            t.update_location()
            #if n==300: t.current_velocity[0]-=3
            #if n == 800: t.current_velocity[1] -= 3

            #if n == 1200: t.current_velocity[0] +=2

            #if n==1000: t.current_velocity[1]-=2
            ...
            #if len(normalized_innovation)>2:
             #   if (abs(normalized_innovation[-1])>1):
              #      print(n)
            ...
            temp = ((tracker_object.innovation_list[-1]))**2/tracker_object.innovation_var[-1]
            temp2 = ((tracker_object.innovation_list[-1])) ** 2 / tracker_object.innovation_var_max[-1]

            prob = (1.0/(np.sqrt(np.pi)*bearing_var))*np.exp(-.5*temp[0])
            prob2 = (1.0/(np.sqrt(np.pi*tracker_object.innovation_var_max[-1])))*np.exp(-.5*temp2[0])


            np1 = prob/(prob2+prob)
            #if np.isnan(np1[0][0]): np1[0][0] = 1
            normalized_innovation.append(temp[0])

            if n>20:
                m1 = np.mean(normalized_innovation[n-10:n])
                m2 = np.mean(normalized_innovation[n-20:n-10])
                normalized_innovation_max.append(m1/m2)

            delta_change_point +=1
            if n>20 and delta_change_point>50:
                if normalized_innovation_max[-1]>10:
                    #print(n)
                    tracker_object.p_k_k+= .5*np.diag([MAX_UNCERTAINTY,MAX_UNCERTAINTY,MAX_UNCERTAINTY,MAX_UNCERTAINTY])
                    delta_change_point = 0
