...
def run(args):
    ...
    while episode_counter<N_max:
        ...
        while episode_condition:
            ...
            extra_information = s.update_location_new_limit(params,input_state,sigma,v_max,coeff,alpha1,alpha2,alpha1_,alpha2_)
        ...
        if episode_condition and training:
            ...
            for e in range(0,len(episode_actions)):
                ...
                if method==0:
                    ...
                    limitted_action1,grad1 = get_limit(v_max,coeff,alpha1,alpha2,alpha1_,alpha2_,predicted_action[0][0])
                    limitted_action2,grad2 = get_limit(v_max, coeff, alpha1, alpha2, alpha1_, alpha2_, predicted_action[1][0])
                    predicted_action[0][0] = limitted_action1
                    predicted_action[1][0] = limitted_action2
                    limit_grad = np.array([grad1,grad2]).reshape([2,1])
                    #gradiant = ((episode_actions[e].reshape(2,1)-predicted_action).dot(state.transpose()))/sigma**2#This is the gradiant
                    gradiant = ((episode_actions[e].reshape(2,1)-predicted_action)*limit_grad).dot(state.transpose())/sigma**2
                elif method==1:
...
if __name__=="__main__":
    ...
    p = Pool(15)
    experiment_folder_name = "linear_policy_discrete_reward_initial_condition_limit_vmax7_coeff9_varying_var"
    ...
    job_args = [(method,RBF_components,MLP_neurons,i,experiment_folder_name,vel_var,coeff,v_max) for i in range(0,15)]
    p.map(run,job_args)
